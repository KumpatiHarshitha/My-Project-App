...
function IceCreams() {
   const iceCreamsProducts = useSelector(globalState => globalState.products.iceCreams);
    ...
    const[selectedRange,setSelectedRange]=useState([])
      const priceRanges=[
       ...
       const activeRanges=priceRanges.filter(range=>selectedRange.includes(range.value));
         ...
         iceCreamsProducts:iceCreamsProducts.filter(product=>
...
const totalPages = Math.ceil(filtered.length / itemsPerPage);
   const iceCreamsListItems = paginatedItems.map((product, index) => (
      <li key={index} className="IceCreams-item">
    ...
    return (
      <>
      <div className="IceCreams-container">
